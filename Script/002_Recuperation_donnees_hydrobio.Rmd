...
```{r}
stations_hydrobio <- telecharger_stations(code_departement, station_suivie)
```
...
```{r}
indices_hydrobio <- telecharger_indices(code_departement, station_suivie, code_indice, annee_depart)%>%
...
```{r}
taxons_hydrobio <- telecharger_listes( code_departement, station_suivie, code_eqb, annee_depart)
```
...
## Sauvergarder les données au format .csv dans notre répertoire
```{r}
# Sauvegarder le résutat au format .rds
saveRDS(stations_hydrobio, "stations_hydrobio.rds")
saveRDS(indices_hydrobio, "indices_hydrobio.rds")
saveRDS(taxons_hydrobio, "taxons_hydrobio.rds")
```
